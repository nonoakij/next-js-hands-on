...
ソフト ナビゲーション（Next Link）では、遷移先の URL と一致するサブページ(今回でいうと`@header/sent/page.tsx`)がなくても、現在のサブページを維持して遷移します。
ハード ナビゲーション(ブラウザーのリロードや直接アクセスされた時) は、 URL と一致するサブページがない場合は、`default.js` が表示されます。`default.js` がない場合は、404 が表示されます。
